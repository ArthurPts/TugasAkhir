...
it('transpose tidak mengubah chord_id yang tersimpan di database', function () {
    ...
    Queue::assertPushed(GenerateChordAudioJob::class, function (GenerateChordAudioJob $job) {
        return $job->chord->name === 'C#' && $job->voice === 'en-US-AriaNeural';
    });
});
...
it('transpose satu lagu penuh mengembalikan seluruh marker ter-transpose tanpa mengubah db', function () {
    Storage::fake('public');
    Storage::disk('public')->put('chord-audio/c-sharp.mp3', 'fake audio');
    Storage::disk('public')->put('chord-audio/c-minor.mp3', 'fake audio');
    ...
    $song = Song::create([
        ...
        'file_path' => 'songs/song-sample.mp3',
        'visibility' => 'public',
    ]);
    ...
    $transposedFirst->update(['file_path' => 'chord-audio/c-sharp.mp3']);
    ...
    $transposedSecond->update(['file_path' => 'chord-audio/c-minor.mp3']);
    ...
    $response->assertOk()
        ->assertJsonPath('song.file_path', 'songs/song-sample.mp3')
